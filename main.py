...
def main():
    st.set_page_config("Horarios", "📚", layout="wide")
    ...
    with st.sidebar:
        ...
        clases = list(gen.prolog.query("clase(_,_,_)"))
        franjas = list(gen.prolog.query("franja(_)"))
        aulas = list(gen.prolog.query("aula(_)"))
        st.metric("Clases", len(clases))
        st.metric("Franjas", len(franjas))
        st.metric("Aulas", len(aulas))

    if st.button("Generar Horario"):
        horario = gen.generar_horario()

        if horario:
            st.success("Horario generado con éxito")
            df = pd.DataFrame(horario)
            st.dataframe(df[["clase", "profesor", "franja", "aula", "duracion"]])

            st.plotly_chart(px.bar(df, x="profesor", y="duracion", color="clase"))
            st.plotly_chart(px.pie(df, names="aula"))

            csv = df.to_csv(index=False)
            st.download_button("Descargar CSV", csv, "horario.csv")
        else:
